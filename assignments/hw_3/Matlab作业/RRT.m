...
for iter = 1:3000
    ...
    length = xL^2 + yL^2;
    for j = 1: size(T.v, 2)
        l = (x_rand(1) - T.v(j).x)^2 + (x_rand(2) - T.v(j).y)^2;
        if l < length
            x_near(1) = T.v(j).x;
            x_near(2) = T.v(j).y;
            length = l;
    ...
    step = x_rand - x_near;
    step_size = norm(step);
    x_new = x_near + min(Delta/step_size, 1) * step;
    ...
    T.v(j+1).x = x_new(1);
    T.v(j+1).y = x_new(2);
    T.v(j+1).xPrev = x_near(1);     % 起始节点的父节点仍然是其本身
    T.v(j+1).yPrev = x_near(2);
    T.v(j+1).dist = norm(x_new-x_near);
    T.v(j+1).indPrev = ind;

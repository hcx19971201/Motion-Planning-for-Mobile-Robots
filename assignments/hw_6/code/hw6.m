...
for k=1:n_seg
    f1 = plot(x_pos((k-1)*100+k:k*101),y_pos((k-1)*100+k:k*101),'DisplayName','Bezier Curves');
    f1.Color = color(k);
    scatter(poly_coef_x((k-1)*(n_order+1)+1:k*(n_order+1))*ts(k),...
            poly_coef_y((k-1)*(n_order+1)+1:k*(n_order+1))*ts(k),color(k));
end

...
{ 
    ...
    */
    ...
    double minimum_path = _Vel * _Vel /  _Acc;

    for (int i = 0; i < Path.rows() - 1; i++)
    {
        VectorXd path = Path.row(i+1) - Path.row(i);
        double s = path.norm();

        if (s < minimum_path)
        {
            time(i) = 2 * std::sqrt(s / _Acc);
        }
        else
        {
            time(i) = _Vel / _Acc * 2 + (s - minimum_path) / _Vel;
        }
        
        cout << "Time for part " << i << ": " << time(i) << endl;
    }
